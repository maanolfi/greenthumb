...
export default function Details() {
  ...
  const titleSvgSun = NamesSvgGrey.filter(elem =>
    elem.includes(titleIconSun.replace(/([\u0300-\u036f]|[^0-9a-zA-Z])/g, '-'))
  ).toString();

  const titleSvgWater = NamesSvgGrey.filter(elem =>
    elem.includes(
      titleIconWater.replace(/([\u0300-\u036f]|[^0-9a-zA-Z])/g, '-')
    )
  ).toString();

  const titleSvgToxicOrPet = NamesSvgGrey.filter(elem =>
    elem.includes(
      titleIconPetorToxic.replace(/([\u0300-\u036f]|[^0-9a-zA-Z])/g, '-')
    )
  ).toString();

  const upperCase = text => {
    const str1 = text.substring(0, 1);
